...
class GameObject {
 ...
 protected:
  void checkBoundary() {
    ...
    if (pos.x + width < 0)
      pos.x = MAX_X + width;  // left side
    else if (pos.x - width > MAX_X)
      pos.x = -width;  // right side

    if (pos.y + height < 0)
      pos.y = MAX_Y + height;  // top
    else if (pos.y - height > MAX_Y)
      pos.y = -height;  // bottom
...
class Player : public GameObject {
 public:
  ...
  Player(sf::Vector2f p, sf::Vector2f v) : GameObject(p, v) {
...
class Asteroid : public GameObject {
 public:
  ...
  Asteroid(int s, sf::Vector2f p, sf::Vector2f v) : GameObject(p, v) {
    stage = s;
    loadShape(s);
  }
  ...
  Asteroid(int s)
      : Asteroid(s, getRandomPos(), getRandomDirection(),
                 getRandomSpeed(4, 6)) {}
 ...
 private:
  void loadShape(int stage) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      shape.setPoint(i, sf::Vector2f(radius * std::cos(angle * M_PI / 180.0f),
  ...
  static sf::Vector2f getRandomPos() {
    ...
    float posX =
        static_cast<float>(std::rand() % MAX_X);  // Adjust window width
    float posY =
        static_cast<float>(std::rand() % MAX_Y);  // Adjust window height
    return sf::Vector2f(posX, posY);
  }

  static sf::Vector2f getRandomVel() {
    // Set a random velocity
    float velX = static_cast<float>(std::rand() % 11) -
                 5;  // Random value between -5 and 5
    float velY = static_cast<float>(std::rand() % 11) -
                 5;  // Random value between -5 and 5
 ...
 public:
  ...
  void update() {
    ...
    for (auto& particle : particles) {
      float angle = std::atan2(particle.getPosition().y - position.y,
...
class Game {
 public:
  ...
  Game()
    ...
    font.loadFromFile("/System/Library/Fonts/Supplemental/Verdana.ttf");
 ...
 private:
  ...
  void handle_input() {
    ...
    while (window.pollEvent(event)) {
      ...
      if (controls.getState("quit")) window.close();
    }
  }
  ...
  void update() {
    ...
    if (!player.is_dead) {
      ...
      if (controls.getState("fire")) {
        ...
        {
          shots.push_back(
              std::make_shared<Shot>(player.getPos(), player.getAngle()));
          controls.setState("fire", 0);
        }
      }
    }
    ...
    info_text.setString(
        "LEVEL: " + to_string(level) + "   SHIPS: " + to_string(lives) +
        ...
        if (obj1->getHitbox().intersects(obj2->getHitbox())) {
          if (asteroids.size() > 1)
            explosions.push_back(
                std::make_shared<Explosion>(obj1->getPos(), 10, 3.0f, 15));
          else
            explosions.push_back(
                std::make_shared<Explosion>(obj1->getPos(), 10, 3.0f, 45));
          ...
          if (obj1->stage > 1) {
            ...
            new_asteroids.push_back(std::make_shared<Asteroid>(
                new_stage, new_pos, new_angle - 55.0, new_speed));
            new_asteroids.push_back(std::make_shared<Asteroid>(
                new_stage, new_pos, new_angle + 55.0, new_speed));
          }
  ...
  void asteroid_player_collisions() {
    if (player.is_dead || player.invincible) return;

    for (const auto& obj1 : asteroids) {
      if (obj1->getHitbox().intersects(player.getHitbox())) {
        player.is_dead = true;
        explosions.push_back(
            std::make_shared<Explosion>(player.getPos(), 30, 4.0f, 60));
      }
    }
  }
